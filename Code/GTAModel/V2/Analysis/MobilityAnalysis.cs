...
namespace TMG.GTAModel.V2.Analysis
{
    ...
    public class MobilityAnalysis : ISelfContainedModule
    {
        ...
        {
            BuildTags( this.AgeTagString, out this.AgeTags );
            BuildTags( this.MobilityTagString, out this.MobilityTags );
            return true;
        }

        public void Start()
        {
            using ( StreamWriter writer = new StreamWriter( this.OutputFileName.GetFileName() ) )
            {
                writer.WriteLine( "Occupation,Age,MobilityCategory,Total" );
                for ( int i = 0; i < this.NumberOfOccupations; i++ )
                {
        ...
        {
            var fileName = this.MobilityCacheFile.GetFileName() + ( occupationIndex + ".bin" );
            try
            {
            ...
            {
                throw new XTMFRuntimeException( "In '" + this.Name + "' we were unable to load a file named '" + fileName + "'!" );
            }
        }
        ...
        private void LoadMobilityData(int occupationIndex, IZone[] zones, double[][] ageTotals)
        {
            ...
            {
                var expectedFileSize = sizeof( float ) * this.AgeTags.Length * this.MobilityTags.Length * zones.Length * zones.Length;
                if ( reader.BaseStream.Length < expectedFileSize )
                {
                    throw new XTMFRuntimeException( "In '" + this.Name + "' we were expecting the mobility cache to be '"
                ...
                // for each age
                for ( int age = 0; age < this.AgeTags.Length; age++ )
                {
                    ...
                    {
                        // for each mobility
                        for ( int mobility = 0; mobility < this.MobilityTags.Length; mobility++ )
                        {
        ...
        private void ProcessMobilityData(StreamWriter writer, int occupationIndex)
        {
            ...
            double[][] ageTotals = new double[this.AgeTags.Length][];
            for ( int i = 0; i < ageTotals.Length; i++ )
            {
                ageTotals[i] = new double[this.MobilityTags.Length];
            }
            ...
            for ( int age = 0; age < this.AgeTags.Length; age++ )
            {
                for ( int mobility = 0; mobility < this.MobilityTags.Length; mobility++ )
                {
                    ...
                    writer.Write( ',' );
                    writer.Write( this.AgeTags[age] );
                    writer.Write( ',' );
                    writer.Write( this.MobilityTags[mobility] );
                    writer.Write( ',' );
