...
{
    public class PVMode : ITashaMode
    {
        [RootModule]
        ...
        [RunParameter( "LogDistance", false, "Should we scale against the log base e of distance?" )]
        public bool LogDistance;

        [RunParameter( "ExpDistance", false, "Should we scale against the distance^2?" )]
        ...
        public double CalculateV(ITrip trip)
        {
            ...
            float distance;
            object distanceObject = trip[this.DistanceName];
            ...
            {
                distance = (float)distanceObject;
            }
            // Calculate the time of travelling
            var v = 0f;
            v = this.LinearTravelTimeWeight * ( distance + this.ModeConstant );
            if ( this.LogDistance )
            {
                v += this.NonLinearTravelTimeFactor * this.TravelTimeFactor * ( (float)Math.Log( distance ) + this.ModeConstant );
            }
            else if ( this.ExpDistance )
            {
                v += this.NonLinearTravelTimeFactor * (float)( ( distance * distance ) + this.ModeConstant );
            }
            // calculate the cost and the scale to convert it into minutes
            v += this.TravelCostWeight * ( this.TravelCostFactor * distance );
            // calculate the cost and the scale to convert it into minutes
            v += this.TravelCostWeight * ( this.TravelCostFactor * distance );
        ...
        public bool RuntimeValidation(ref string error)
        {
            ...
            if ( this.Root == null )
            {
                error = "There was no root attached to the model system!";
                return false;
            }
            // Find vehicle type
