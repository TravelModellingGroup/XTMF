        ...
        public static void FlagIfGreaterThan(float[] dest, float value, float[] data)
        {
            ...
            if(Vector512.IsHardwareAccelerated)
            {
                ...
                for (i = 0; i < data.Length - Vector512<float>.Count; i += Vector512<float>.Count)
                {
                    ...
                    var local = Vector512.ConditionalSelect(Vector512.GreaterThan(vData, vValue), one, zero);
            ...
            if(Vector512.IsHardwareAccelerated)
            {
                ...
                if ((destIndex | lhsIndex | rhsIndex) == 0)
                {
                    ...
                    for (; i <= length - Vector512<float>.Count; i += Vector512<float>.Count)
                    {
                        ...
                        var local = Vector512.ConditionalSelect(Vector512.GreaterThan(f, s), one, zero);
