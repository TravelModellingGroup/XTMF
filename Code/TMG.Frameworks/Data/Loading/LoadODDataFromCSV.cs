...
public class LoadODDataFromCSV : IReadODData<float>
{
    ...
    [RunParameter("Read Type", ReadType.AutoDetect, "Either auto detect if we are reading a vector or a matrix or specify this manually.  This only applies to ThirdNormalized.", Index = 0)]
    public ReadType ThirdNormalizedType;

    [RunParameter("Format", FileType.ThirdNormalized, "The format to read the data from.", Index = 1)]
    public FileType CSVFormat;

    [RunParameter("Origin Column", 0, "The column to use for the origin if this is ThirdNormalized. If auto detect is used, this will be ignored.", Index = 2)]
