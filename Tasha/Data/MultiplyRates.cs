...
namespace Tasha.Data
{
    ...
    public class MultiplyRatesForZones : IDataSource<SparseArray<float>>
    {
        ...
        public bool RuntimeValidation(ref string error)
        {
            ...
            {
                error = "In '" + this.Name + "', if you save rates by PD the input rates must be read in by PD.  The first rate is being read in by zone!";
                return false;
            }
            else if(SaveRatesBasedOnPD & !SecondRateBasedOnPD)
            {
                error = "In '" + this.Name + "', if you save rates by PD the input rates must be read in by PD.  The second rate is being read in by zone";
                return false;
            }
            return true;
        }
