...
public sealed class BicycleWithImpedance : ITashaMode, IIterationSensitive
{
    ...
    public sealed class TimePeriodData : IModule
    {

        [RunParameter("Start Time", "6:00", typeof(Time), "The start time for this data.", Index = 0)]
        public Time StartTime;

        [RunParameter("End Time", "9:00", typeof(Time), "The start time for this data.", Index = 1)]
        public Time EndTime;

        [SubModelInformation(Required = true, Description = "The time, in minutes, it takes to go from origin to destination.")]
        public IDataSource<SparseTwinIndex<float>> TravelTime;
    ...
    [SubModelInformation(Required = true, Description = "Travel times, utility, and distance by time of activity.")]
    public TimePeriodData[] PeriodData;
    ...
    {
        throw new NotImplementedException("The interface for trip based models is not supported at this time.");
    }
    ...
    {
        var data = GetData(timeOfDay);
        return data._distances[origin.ZoneNumber, destination.ZoneNumber] <= MaxTravelDistance;
    }

    [RunParameter("Default Time Period", 0, "The time period to use if outside of all defined time periods.")]
    public int DefaultTimePeriod;

    private TimePeriodData GetData(Time time)
    {
        ...
        return PeriodData[DefaultTimePeriod];
    }
    ...
    {
        var data = GetData(trip.ActivityStartTime);
        return data._distances[trip.OriginalZone.ZoneNumber, trip.DestinationZone.ZoneNumber] <= MaxTravelDistance;
    }
    ...
    public bool RuntimeValidation(ref string error)
    {
        if(DefaultTimePeriod < 0 || DefaultTimePeriod >= PeriodData.Length)
        {
            error = "The default time period is not valid!";
            return false;
        }
        return true;
    }
    ...
    public Time TravelTime(IZone origin, IZone destination, Time time)
    {
        ...
        var travelTime = data._travelTime[origin.ZoneNumber, destination.ZoneNumber];
    ...
    {
        var data = GetData(time);
        var travelTime = data._travelTime.GetFlatData()[flatOrigin][flatDesination];
        return Time.FromMinutes(travelTime);
    }
